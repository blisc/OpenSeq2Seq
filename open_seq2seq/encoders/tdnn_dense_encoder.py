...
class TDNNDenseEncoder(Encoder):
  ...
  def __init__(self, params, model, name="w2l_encoder", mode='train'):
    ...
    * **bn_epsilon** (float) --- epsilon for batch norm. Defaults to 1e-3.
    """
    super(TDNNDenseEncoder, self).__init__(params, model, name, mode)
  ...
  def _encode(self, input_dict):
    ...
    for idx_convnet in range(len(convnet_layers)):
      ...
      if layer_repeat > 1:
        ...
        if self.params.get("dense_mode", "dense") == "denser":
          residual_aggregation.append(conv_feats)
      ...
      if layer_repeat > 1:
        conv_feats = tf.concat(conv_dense, axis=-1)
        if self.params.get("dense_mode", "dense") == "denser" and len(layer_res) > 0:
